...
dir_path = r"/home/preeti/Desktop/Question_Tag_Assignment/ds-data"
...
def collect_data(dir_path):
    print("collecting all json files from the" + dir_path)
    ...
    print("collecting all the json files in single dataframe")
    dfList = []
...
def collect_single_data():
    ...
    p1=r"/home/preeti/Desktop/Question_Tag_Assignment/ds-data/04-09-202220_53_24T/377801-377316.json"
    with open(p1) as f1:
        data=json.load(f1)
...
def cleanup_data(df):
    print("cleanup_data: using only id and text fields and doing text pre-processing")
    df=df[['id','text']]
    ...
    df.to_csv(dir_path+'/non_processed.csv',index=False)

    #pre process the text
    stplist = stoplist.difference(question_words)
    df['text']=df['text'].apply(lambda x:pre_process(stplist,x))
    df.to_csv(dir_path+'/processed.csv',index=False)
    return df

def get_questions(df):
    print("get_questions: identify questions using non-zero tfidf feature values for question_words vocabulary")
    ...
    dfq=df.iloc[nz1]
    dfq.to_csv(dir_path+'/q1.csv',index=False)
    return dfq
...
# we use gensim stop-words and add my own stop-words, based on texts
...
pp(result.head(5))
result.to_csv(dir_path+'/tag_result.csv',index=False)
